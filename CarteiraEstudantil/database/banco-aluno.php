...
function editarAluno($conexao, $nome, $matricula, $codigo, $datanascimento, $rg, $instituicao, $curso, $fotoID){
    $query = "UPDATE aluno SET matricula = {$matricula}, code_acess = '{$codigo}', nome = '{$nome}', nascimento = '{$datanascimento}', rg = {$rg}, fotoID = '{$fotoID}', curso = {$curso}, instituicao = {$instituicao} WHERE matricula = {$matricula};";
    return mysqli_query($conexao, $query);
}
